...
with open('../data/VUAsequence/VUA_seq_formatted_train.csv', encoding='latin-1') as f:
    ...
    next(lines)
    for line in lines:
        label_seq = ast.literal_eval(line[3])   # 0,1 label로 이루어진 sequence
        assert (len(line[2].split()) == len(label_seq))    # [2]는 space로 띄여진 sentence
        raw_train_vua.append([line[2], label_seq])
...
with open('../data/VUAsequence/VUA_seq_formatted_val.csv', encoding='latin-1') as f:
    ...
    for line in lines:
        ...
        assert (len(line[2].split()) == len(label_seq))    # [2]는 space로 띄여진 sentence
        raw_val_vua.append([line[2], label_seq])
...
embed the datasets
'''
# raw_train_vua: sentence, label_seq
# embedded_train_vua: embedded_sentence, labels
# glove로 만든 nn.embedding 통과한게 튀어나옴
embedded_train_vua = [[embed_indexed_sequence(example[0], word2idx, glove_embeddings, elmos_train_vua), example[1]]
                      for example in raw_train_vua]
embedded_val_vua = [[embed_indexed_sequence(example[0], word2idx, glove_embeddings, elmos_val_vua), example[1]]
...
for epoch in range(num_epochs):
    ...
    for (example_text, example_lengths, labels) in train_dataloader_vua:
        ...
        if num_iter % 200 == 0:
            avg_eval_loss = evaluate(val_dataloader_vua, RNNseq_model, loss_criterion, using_GPU)
